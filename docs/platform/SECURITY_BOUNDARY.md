...
disabled and browser PUT/GET/HEAD/OPTIONS enabled. The approved local mode is
`global_api_local_development`, limited to localhost:5173 with
`bucket_policy_applied: false`; the bucket is private. Production S3 must
configure its own CORS policy. Runtime evidence reports GET and HEAD only when
those Origin requests are actually observed.
