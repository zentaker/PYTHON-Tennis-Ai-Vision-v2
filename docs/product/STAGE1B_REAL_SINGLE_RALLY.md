...
`STAGE1B_FIXTURE_PUBLICATION_SAFETY_PATCH_IMPLEMENTED`,
`STAGE1B_RELEASE_AUDIT_PASSED` and
`REAL_SINGLE_RALLY_INTEGRATION_GATE_PASSED` are recorded. The first real bundle
is accepted for TennisWebAI Stage 0B consumption. The source remains external,
session/rally remain `partial`, surface remains `unknown`, and this acceptance
contains no 3D or inference claim.
