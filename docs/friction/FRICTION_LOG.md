...
| F-0005 | 2026-05-18 | 1 | DEP | `ffmpeg` no esta instalado en WSL al iniciar 1.1, aunque Stage 1 sugeria usarlo para extraer `reference_frame.png`. | 0.1 | Extraido frame 0 con OpenCV desde el entorno Python ya validado. Instalar `ffmpeg` queda pendiente para etapas que lo requieran. | Resuelto |
| F-0006 | 2026-05-18 | 1 | ENV | WSLg/OpenCV GUI no pudo abrir `cv2.imshow`: Qt encontro plugin `xcb` pero no pudo inicializar plataforma grafica. | 0.2 | Activado modo `1.3-fallback`: el usuario reporta manualmente los 8 pixeles sobre `reference_frame.png`; el agente persiste JSON y preview. | Resuelto |
