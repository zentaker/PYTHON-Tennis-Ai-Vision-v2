...
| MMCV | 2.1.0 | Official `cu121/torch2.3` wheel index; MMDetection 3.2.x requires `<2.2.0`. |
| MMDetection | 3.2.0 | Compatible with MMPose 1.3.2 (`mmpose` declares `<3.3.0`). |
...
index explicitly so it cannot silently compile MMCV from source.
