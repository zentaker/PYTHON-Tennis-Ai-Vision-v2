...
def main() -> int:
    ...
    if not hypotheses:
        ...
        hypotheses = [{"hypothesis_id": "H1_FALLBACK", "total_cost": float("inf"), "candidate_ids": [c["candidate_id"] for c in fallback_choices], "event_frames": [c["frame_id"] for c in fallback_choices], "choices": fallback_choices, "rms_node_difference": 0.0, "ambiguity": False}]
    if len(hypotheses) == 1:
