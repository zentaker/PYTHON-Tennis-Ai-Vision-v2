...
def canonical_frame(frame: np.ndarray) -> np.ndarray:
    return cv2.rotate(frame, cv2.ROTATE_90_COUNTERCLOCKWISE) if frame.shape[0] > frame.shape[1] else frame
...
def draw_court(image: np.ndarray, matrix: np.ndarray, color: tuple[int, int, int], thickness: int) -> None:
    for start, end in COURT_LINES.values():
        pixels = apply_homography(matrix, np.asarray([start, end]))
        cv2.line(image, tuple(np.rint(pixels[0]).astype(int)), tuple(np.rint(pixels[1]).astype(int)), color, thickness)
...
def main() -> None:
    ...
    result = refine_homography(initial, distance, court_points, pixel_points, config["regularization"])
    ...
    line_report = {
        ...
        "lines_used": list(COURT_LINES), "lines_rejected": [],
        ...
        "per_line": per_line, "condition": result.condition,
    ...
    extended_h = {
        "schema_version": "1.0", "stage": "5A.2", "clip_id": "nivel_a2_01",
        "coordinate_system": "X right, Y far, ground Z=0, metres",
        "H_court_to_pixel": refined.tolist(), "H_pixel_to_court": inverse_refined.tolist(),
        "source_stage1_sha256": digest(H_PATH), "condition": result.condition,
        "line_at_infinity": result.infinity_line,
    }
    write_json(OUTPUT / "extended_ground_homography.json", extended_h)
    extended_camera = camera.to_dict(
        stage="5A.2", status="GROUND_PARAMETERS_PRESERVED_FROM_STAGE5A1",
    ...
    for _ in range(config["uncertainty_runs"]):
        ...
        bootstrap.append(apply_homography(np.linalg.inv(candidate), apply_homography(initial, probes)))
    bootstrap_array = np.asarray(bootstrap)
    uncertainty_points = []
    for index, probe in enumerate(probes):
        values = bootstrap_array[:, index]
        uncertainty_points.append({
            "ground_xy": probe.tolist(), "uncertainty_x_m": float(np.std(values[:, 0])),
    ...
    uncertainty = {
        "runs": config["uncertainty_runs"], "seed": config["seed"],
        "perturbations": ["line pixels", "corners", "vertical reference", "resolution", "initial camera"],
        "implemented_perturbations": ["line/corner pixels"],
        "unavailable_perturbations": ["vertical reference samples not serialized per frame", "resolution fixed canonical", "camera prior preserved"],
    ...
    contacts = audit_payload if isinstance(audit_payload, list) else audit_payload.get("contacts", audit_payload.get("events", []))
    audits = {int(row["frame_id"]): row for row in contacts}
    rows, comparisons = [], []
    camera_h = camera_homography(camera)
    consistency_probes = np.asarray([(x, y) for x in (-5.485, 0, 5.485) for y in (-16.885, -11.885, 0, 11.885, 16.885)])
    ...
    for pose in poses:
        ...
        row = {
            "frame_id": frame_id, "event_id": audit.get("event_id"),
            "track_id": pose["track_id"], "identity": identity,
            "foot_pixel_estimate": foot["pixel"], "pixel_uncertainty": foot["pixel_uncertainty"],
            "supporting_keypoints": foot["supporting_keypoints"], "temporal_support": foot["temporal_support"],
            "homography_xy_m": hxy.tolist(), "camera_ray_ground_xy_m": camera_xy.tolist(),
            "method_disagreement_m": fusion["method_disagreement_m"], "fused_xy_m": fusion["fused_xy"],
            "reported_xy_m": chosen, "metric_uncertainty_m": fusion["metric_uncertainty_m"],
            "confidence": float(pose["confidence"]) / (1 + fusion["metric_uncertainty_m"]),
            "resolved": fusion["resolved"], "baseline_distance_m": float(distance_baseline),
            "warnings": foot["warnings"] + fusion["warnings"],
        }
        rows.append(row)
        stored = audit.get("court_position", {})
        comparisons.append({
            "event_id": row["event_id"], "frame_id": frame_id,
            "stored_p1_xy_m": [stored.get("x_m"), stored.get("y_m")],
            "refined_homography_xy_m": row["homography_xy_m"],
            "camera_xy_m": row["camera_ray_ground_xy_m"], "fused_xy_m": row["fused_xy_m"],
            "baseline_distance_m": row["baseline_distance_m"], "uncertainty_m": row["metric_uncertainty_m"],
        })
    ...
    report = {
        "frames_processed": len(rows), "players_resolved": resolved,
        "players_unresolved": len(rows) - resolved, "maximum_method_disagreement_m": max(ground_disagreements),
        "zero_identity_changes": True, "zero_nonfinite_positions": bool(np.isfinite(ground_disagreements).all()),
        "events": {row["event_id"]: {"baseline_distance_m": row["baseline_distance_m"], "uncertainty_m": row["metric_uncertainty_m"], "resolved": row["resolved"]} for row in rows},
    }
    write_json(OUTPUT / "player_ground_position_report.json", report)
    write_json(OUTPUT / "coordinate_comparison.json", comparisons)
    camera_report = {
        "checks": ["camera_to_plane", "plane_to_image", "homography_to_image", "ray_ground", "method_agreement"],
        "probe_count": len(consistency_probes), "median_pixel_disagreement": float(np.median(pixel_disagreement)),
    ...
    overlay = background.copy(); overlay[mask > 0] = (255, 200, 0)
    draw_court(overlay, initial, (0, 0, 255), 3); draw_court(overlay, refined, (0, 255, 0), 3)
    save_jpg("stage5a2_court_line_overlay.jpg", overlay)
    old_new = background.copy(); draw_court(old_new, initial, (0, 0, 255), 5); draw_court(old_new, refined, (0, 255, 0), 2)
    save_jpg("stage5a2_old_vs_refined_homography.jpg", old_new)
    consistency = background.copy(); draw_court(consistency, refined, (0, 255, 0), 4); draw_court(consistency, camera_h, (255, 0, 255), 2)
    save_jpg("stage5a2_camera_homography_consistency.jpg", consistency)
    contact = background.copy()
    for row in rows:
        center = tuple(np.rint(row["foot_pixel_estimate"]).astype(int)); cv2.circle(contact, center, 12, (0, 255, 255), -1)
        cv2.putText(contact, f"{row['event_id']} {row['identity']}", (center[0] + 15, center[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 3)
        cv2.putText(contact, f"{row['event_id']} {row['identity']}", (center[0] + 15, center[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 1)
    save_jpg("stage5a2_player_foot_contact_sheet.jpg", contact)
    fig, ax = plt.subplots(figsize=(10, 12)); ax.set_xlim(-9, 9); ax.set_ylim(-17.5, 17.5); ax.set_aspect("equal")
    for start, end in COURT_LINES.values(): ax.plot([start[0], end[0]], [start[1], end[1]], "k-", linewidth=0.8)
    for row in rows:
        xy = row["reported_xy_m"]; ax.errorbar(xy[0], xy[1], xerr=row["metric_uncertainty_m"], yerr=row["metric_uncertainty_m"], fmt="o", label=row["event_id"])
    ax.axhline(16.885, color="r", linestyle="--"); ax.axhline(-16.885, color="r", linestyle="--"); ax.legend(); ax.set_title("Stage 5A.2 player ground positions (± uncertainty)")
    fig.savefig(OUTPUT / "stage5a2_player_ground_top_view.jpg", dpi=150, bbox_inches="tight"); plt.close(fig); shutil.copy2(OUTPUT / "stage5a2_player_ground_top_view.jpg", ASSETS / "stage5a2_player_ground_top_view.jpg")
    fig, ax = plt.subplots(figsize=(11, 6)); ys = [point["ground_xy"][1] for point in uncertainty_points]; values = [point["uncertainty_y_m"] for point in uncertainty_points]
    ax.plot(ys, values, "o-"); ax.axvline(-11.885, color="k", linestyle="--"); ax.axvline(11.885, color="k", linestyle="--"); ax.set(xlabel="court Y (m)", ylabel="Y uncertainty (m)", title="Deterministic calibration perturbation uncertainty")
    fig.savefig(OUTPUT / "stage5a2_extrapolation_uncertainty.jpg", dpi=150, bbox_inches="tight"); plt.close(fig); shutil.copy2(OUTPUT / "stage5a2_extrapolation_uncertainty.jpg", ASSETS / "stage5a2_extrapolation_uncertainty.jpg")

    far_usable = all(row["resolved"] and row["metric_uncertainty_m"] <= 1.5 and row["baseline_distance_m"] <= 5 for row in rows if row["identity"] == "far")
    line_gate = line_report["refined_median_px"] <= 4 and line_report["refined_p95_px"] <= 10
    status = "STAGE5A2_EXTENDED_GROUND_PLANE_READY_FOR_HUMAN_GATE" if far_usable and line_gate else "STAGE5A2_EXTENDED_GROUND_PLANE_PARTIAL"
    validation = {
        "status": status, "line_gate_passed": line_gate, "far_positions_usable": far_usable,
        "human_visual_approval": "pending", "deterministic_seed": config["seed"],
        "no_xyz_reconstruction_run": True, "cloud_calls": 0, "gpu_calls": 0, "spend": 0,
    }
    ...
    manifest = {
        "schema_version": "1.0", "clip_id": "nivel_a2_01", "image_resolution": [2746, 1536],
        "coordinate_system": "canonical rotated horizontal pixels; court X right/Y far metres",
        "inputs": [{"path": str(path), "sha256": digest(path), "bytes": path.stat().st_size, "accepted": True} for path in manifest_paths],
        "video_frames": video_frames, "missing_inventory": ["original extracted frames", "per-frame serialized net vertical reference"],
    }
    write_json(ROOT / "config/ground_plane_calibration/input_manifest.json", manifest)
    write_json(OUTPUT / "stage5a2_run_report.json", {**validation, **report, "video": str(VIDEO), "background_samples": len(sampled_frames)})
    (OUTPUT / "run.log").write_text(f"status: {status}\nCPU-only; XYZ not executed; seed={config['seed']}\n", encoding="utf-8")
