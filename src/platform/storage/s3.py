...
class S3ObjectStorage:
    def __init__(self, settings) -> None:
        ...
        self.internal_client = boto3.client(
            "s3", endpoint_url=settings.s3_internal_endpoint_url, **kwargs
        )
        self.public_signer = boto3.client(
            "s3", endpoint_url=settings.s3_public_endpoint_url, **kwargs
        )
    ...
    def create_presigned_download(
        ...
        url = self.public_signer.generate_presigned_url(
            "get_object", Params=params, ExpiresIn=self.settings.s3_presign_expires_seconds
        )
    ...
    def delete_object(self, key: str) -> None:
        self.internal_client.delete_object(Bucket=self.settings.s3_bucket, Key=self._key(key))
    ...
    def get_bytes(self, key: str) -> bytes:
        return self.internal_client.get_object(Bucket=self.settings.s3_bucket, Key=self._key(key))[
            "Body"
        ].read()

    def bucket_exists(self) -> bool:
        try:
            self.internal_client.head_bucket(Bucket=self.settings.s3_bucket)
